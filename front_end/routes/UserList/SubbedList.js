...
var get_sub_list = (state, props) => state.list[`subbed_list/${props.params.id}`];
var get_url = (state, props) => '/api/update_subbed_list?_id=' + props.params.id;
...
var mapStateToProps = createSelector(
    ...
    (users, user, sub_list, url) => {
        ...
        if (user) {
            var title = user._id == window.user_id ? '订阅我的人' : `订阅${user.nickname}的人`;
        }
        return {
            url,
            users,
